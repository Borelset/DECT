...
#define SECONDARY_DJW 0 /* standardization, off by default until such time. */
                               ...
                               */
#define GENERIC_ENCODE_TABLES \
  0    /* code features.  This is tested but not recommended */
      ...
      if (inst->size <= 18 && inst->size >= 4) {
        ...
        if ((prev != NULL) && (prev->type == XD3_ADD) && (prev->size <= 4)) {
          if ((inst->size <= 6) && (mode <= 5)) {
            prev->code2 =
                163 + (mode * 12) + (3 * (prev->size - 1)) + (inst->size - 4);
  ...
  for (s = 0; s < 6; s += 1) {
    for (i = 0; i < 256; i += 1) {
      switch (s) {
        case 0:
          *str++ =
              (code_table[i].type1 >= XD3_CPY ? XD3_CPY : code_table[i].type1);
          break;
        case 1:
          *str++ =
              (code_table[i].type2 >= XD3_CPY ? XD3_CPY : code_table[i].type2);
          break;
        case 2:
        ...
        case 4:
          *str++ =
              (code_table[i].type1 >= XD3_CPY ? code_table[i].type1 - XD3_CPY
                                              : 0);
          break;
        case 5:
          *str++ =
              (code_table[i].type2 >= XD3_CPY ? code_table[i].type2 - XD3_CPY
                                              : 0);
          break;
      }
    }
...
static int xd3_apply_table_encoding(xd3_stream *in_stream, const uint8_t *data,
  ...
  if ((ret = xd3_decode_memory(data, size, dflt_string, CODE_TABLE_STRING_SIZE,
                               code_string, &code_size, CODE_TABLE_STRING_SIZE,
                               0))) {
    return ret;
  }
...
static int xd3_check_pow2(usize_t value, usize_t *logof) {
  usize_t x = 1;
  usize_t nolog;
  if (logof == NULL) {
    logof = &nolog;
...
static usize_t xd3_pow2_roundup(usize_t x) {
  usize_t i = 1;
  while (x > i) {
    i <<= 1U;
  }
...
static void *__xd3_alloc_func(void *opaque, usize_t items, usize_t size) {
...
static void *xd3_alloc(xd3_stream *stream, usize_t elts, usize_t size) {
  ...
  if (a != NULL) {
    ...
    IF_DEBUG2(
        DP(RINT "[stream %p malloc] size %u ptr %p\n", stream, elts * size, a));
  } else {
...
int xd3_config_stream(xd3_stream *stream, xd3_config *config) {
  ...
  stream->srcwin_maxsz =
      config->srcwin_maxsz ? config->srcwin_maxsz : XD3_DEFAULT_SRCWINSZ;
...
static int xd3_emit_hdr(xd3_stream *stream) {
  ...
  if (stream->current_window == 0) {
    ...
    int use_gencodetbl = GENERIC_ENCODE_TABLES && (stream->code_table_desc !=
    ...
    if (use_gencodetbl) {
      ...
      if ((ret =
               xd3_emit_size(stream, &HDR_TAIL(stream), code_table_size + 2)) ||
          (ret = xd3_emit_byte(stream, &HDR_TAIL(stream),
                               stream->code_table_desc->near_modes)) ||
          (ret = xd3_emit_byte(stream, &HDR_TAIL(stream),
                               stream->code_table_desc->same_modes)) ||
          (ret = xd3_emit_bytes(stream, &HDR_TAIL(stream), code_table_data,
                                code_table_size))) {
        return ret;
      }
    }
...
static int xd3_encode_init(xd3_stream *stream, int full_init) {
  ...
  if (full_init) {
    ...
    if (large_comp) {
      usize_t hash_values =
          (stream->srcwin_maxsz / stream->smatcher.large_step);
...
int xd3_decode_memory(const uint8_t *input, usize_t input_size,
  ...
  return xd3_process_memory(0, &xd3_decode_input, 1, input, input_size, source,
...
int xd3_encode_memory(const uint8_t *input, usize_t input_size,
                      ...
                      uint8_t *output, usize_t *output_size,
                      usize_t output_size_max, int flags) {
  return xd3_process_memory(1, &xd3_encode_input, 1, input, input_size, source,
                            source_size, output, output_size, output_size_max,
                            flags);
}
#endif
...
static int xd3_srcwin_setup(xd3_stream *stream) {
  ...
  src->srcbase = stream->match_minaddr;
  src->srclen =
      std::max((usize_t)length, stream->avail_in + (stream->avail_in >> 2));
...
static int xd3_source_match_setup(xd3_stream *stream, xoff_t srcpos) {
  ...
  if (srcpos < frontier_pos && frontier_pos - srcpos > stream->srcwin_maxsz) {
    IF_DEBUG1(DP(RINT "[match_setup] rejected due to srcwin_maxsz "
                      "distance eof=%" Q "u srcpos=%" Q "u maxsz=%u\n",
                 xd3_source_eof(stream->src), srcpos, stream->srcwin_maxsz));
    goto bad;
  }
...
static int xd3_source_extend_match(xd3_stream *stream) {
  ...
  while (stream->match_fwd < stream->match_maxfwd) {
    ...
    tryrem =
        std::min(stream->match_maxfwd - stream->match_fwd, src->onblk - tryoff);
...
static int xd3_srcwin_move_point(xd3_stream *stream, usize_t *next_move_point) {
   ...
   * maximum window size. */
  logical_input_cksum_pos = std::min(
      (stream->total_in + stream->input_position) * 2,
  ...
  while (stream->srcwin_cksum_pos < logical_input_cksum_pos &&
    ...
    if ((ret = xd3_getblk(stream, blkno))) {
      ...
      IF_DEBUG1(DP(RINT "[srcwin_move_point] async getblk return for %" Q "u\n",
                   blkno));
      return ret;
    }

    IF_DEBUG1(DP(RINT "[srcwin_move_point] T=%" Q "u{%" Q "u} S=%" Q "u EOF=%" Q
                      "u %s\n",
    ...
    do {
      uint32_t cksum =
          xd3_lcksum(stream->src->curblk + blkpos, stream->smatcher.large_look);
...
restartloop:
  ...
  if (xd3_iopt_last_matched(stream) > stream->input_position) {
    stream->min_match = std::max(
        MIN_MATCH, 1 + xd3_iopt_last_matched(stream) - stream->input_position);
  } else {
    stream->min_match = MIN_MATCH;
  }
